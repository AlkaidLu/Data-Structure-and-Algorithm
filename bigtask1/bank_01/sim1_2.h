...
{
    // initially assume all tellers finish at ten times closing time
...
void Simulation::RunSimulation(void)
{
    ...
    while (!pq.PQEmpty())
    {
        ...
        if (e.GetEventType() == arrival)
        {
            ...
            //可以且需要插队
            bool isCut=(e.GetCustomerType() == Vip && tellerID >num_vip_window
            ...
            if(isCut){
                ...
                while (!pq.PQEmpty()) 
                {
                    Event t;
                    t=pq.PQDelete();
                    if(flag==0&&t.GetCustomerType()==notVip&&t.GetEventType()==departure
                    &&t.GetTime()-t.GetServiceTime()>e.GetTime()){
                        flag=1;//插队成功
                        cout<<"   ###CUT###"<<endl;
                        cut_time=t.GetTime()-t.GetServiceTime();
                    }
                    pq_t.PQInsert(t);
                }
                //把临时队列中的传回模拟队列里
                while (!pq_t.PQEmpty())              
                {                                  
                    Event t = pq_t.PQDelete();
                    pq.PQInsert(t);
                }
                //若插队成功，对该arrival事件对应的departure事件进行生成，
                //以及对本队列中的非VIP的离开事件时间进行处理
                if(flag==1){
                    newevent = Event(cut_time,
                            departure, e.GetCustomerType(),e.GetCustomerID(),tellerID,
                            cut_time-e.GetTime(),servicetime);
                    while (!pq.PQEmpty()) 
                    {
                        Event t;
                        t=pq.PQDelete();
                        if(t.GetCustomerType()==notVip&&t.GetEventType()==departure
                        &&t.GetTime()-t.GetServiceTime()>e.GetTime()){
                            t=Event(t.GetTime()+e.GetServiceTime(),
                                departure, t.GetCustomerType(),t.GetCustomerID(),t.GetTellerID(),
                                t.GetWaitTime()+e.GetServiceTime(),t.GetServiceTime());
                        }
                        pq_t.PQInsert(t);
                    }
                    while (!pq_t.PQEmpty())              
