        ...
        else
        {
            ...
            while(!pq.PQEmpty()){
                ...
                &&flag==0&&t.GetisCut()==0){
                    ...
                    if(high_vip_wait<e.GetTime()-(t.GetTime()-t.GetServiceTime()-t.GetWaitTime())){
                        ...
                        vip_ID=t.GetCustomerID();
                        high_wait_arri=t.GetTime()-t.GetServiceTime()-t.GetWaitTime();
                    }
                }
                tmp.PQInsert(t);
            }
            ...
            {                                  
                ...
                if(t.GetCustomerID()==vip_ID){
                    ...
                    to_tellerID=tellerID;
                    //vip_ID=-1;
                    cout<<"VIP customer "<<t.GetCustomerID()<<
                    " will be served by teller "<<tellerID<<" immediately.\n";
                }
                pq.PQInsert(t);
            }
            if(flag==1){
                while(!pq.PQEmpty()){
                    ...
                    //原窗口排在vip后面的其他离开事件预计等待时间减小
                    else if(t.GetTime() - t.GetServiceTime()-t. GetWaitTime()>= high_wait_arri &&t.GetEventType()==departure
